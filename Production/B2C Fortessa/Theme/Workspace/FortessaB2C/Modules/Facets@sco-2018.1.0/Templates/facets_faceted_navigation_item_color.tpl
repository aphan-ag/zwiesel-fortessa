		...
		{{/if}}
			<div class="facets-faceted-navigation-item-color-facet-group-content">
				<ul class="facets-faceted-navigation-item-color-picker">
					{{#each displayValues}}
						<li >
							<a href="{{link}}" title="{{label}}" data-color="{{label}}" class="facets-faceted-navigation-item-color-option {{#if isLightColor}}white-border{{/if}} {{#if isActive}}active{{/if}}" aria-hidden="true">
								{{#if isImageTile}}
									<img
										src="{{image.src}}"
										alt="{{label}}"
										...
										height="{{image.height}}">
								{{else}}
									<span style="background: {{color}}" aria-label="Color {{label}}"></span>
