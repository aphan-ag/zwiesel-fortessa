...
{{/unless}}
	...
	<div class="login-register-register-form-controls-group">
		<label class="login-register-register-form-label" for="register-emailsubscribe">
			<input type="checkbox" name="emailsubscribe" id="register-emailsubscribe" value="T" {{#if isEmailSubscribeChecked}} checked {{/if}}>
			{{translate 'Yes, please sign me up for Fortessa for Home exclusive offers and promotions.'}}
		</label>
	</div>

	<!-- GOOGLE RECAPTCHA V2 -->
	<div class="g-recaptcha" id="g-recaptcha-{{sectionNumber}}" data-sitekey="{{siteKey}}"></div>
	<div id="g-recaptcha-login-message-{{sectionNumber}}"></div>
