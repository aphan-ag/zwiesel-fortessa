<div class="product-views-price">
		...
		{{#if isPriceRange}}
			<span class="product-views-price-range" itemprop="offers">
				...
				<link itemprop="availability"
					href="{{#if isPurchasable}}{{#if isInStock}}https://schema.org/InStock{{else}}{{#if backOrderPreOrder}}https://schema.org/PreOrder{{else}}https://schema.org/InStock{{/if}}{{/if}}{{else}}https://schema.org/OutOfStock{{/if}}" />
			</span>
		...
		{{else}}
			<span class="product-views-price-exact" itemprop="offers">
				...
				<span class="product-views-price-lead" data-rate="{{price}}">
					{{priceFormatted}} {{#if uomData.showPerCase}}<span
							class="product-views-price-lead-per-case">&nbsp;/&nbsp;CASE</span>{{/if}}
				</span>
				{{#if showComparePrice}}
					<small class="product-views-price-old">
						{{comparePriceFormatted}}
	...
	{{else}}
		...
		{{else}}
			<div class="product-views-price-login-to-see-prices">
				<p class="product-views-price-message">
					{{translate disablePricingMessage}}
				</p>
			</div>

		{{/if}}
	{{/if}}
	{{#if uomData.isPurchasable}}
		<div class="pla-uom-cart-line">
			<span>{{translate "Pack Size  - "}}</span>
			<span data-value="custcol_mhi_uom">{{uomData.uom}}</span>
		</div>
	{{/if}}
</div>
