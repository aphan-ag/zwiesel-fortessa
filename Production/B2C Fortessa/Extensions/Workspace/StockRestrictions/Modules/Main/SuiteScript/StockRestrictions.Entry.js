define('StockRestrictions.Entry', [
  'ReorderItems.Model',
  'SC.Models.Init',
  'Application',
  'StoreItem.Model',
  ...
  _.extend(ReorderItemsModel, {
    search: function (order_id, query_filters) {
      ...
      if (items_info.length) {
        ...
        var modelResponse = StockRestrictionsModel.get(_.pluck(items_info, 'id'));

        if (modelResponse && modelResponse.stockInfo) {
          _.each(result.records, function (record) {
            var matchFound = _.find(modelResponse.stockInfo, function (stockInfo) {
              return stockInfo.internalid == record.item.internalid;
            });

            if (matchFound) {
              record.item.quantityAvailableInWinch = matchFound.locationquantityavailable;
              record.item.custitem_mhi_ft_web_safety_stock = matchFound.custitem_mhi_ft_web_safety_stock;
            }
          });

          result.stockInfo = modelResponse.stockInfo;
        }
      }
  ...
  _.extend(OrderHistoryModel, {
    postGet: function () {
      ...
      var modelResponse = StockRestrictionsModel.get(_.pluck(_.pluck(this.result.lines, 'item'), 'internalid'));

      if (modelResponse && modelResponse.stockInfo) {
        _.each(this.result.lines, function (record) {
          var matchFound = _.find(modelResponse.stockInfo, function (stockInfo) {
            return stockInfo.internalid == record.item.internalid;
          });
        ...
        this.stockInfo = modelResponse.stockInfo;
      }
